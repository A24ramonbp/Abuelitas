 ...
 * @author rpbp
 */
public class UserPersonalManagerController {

    private final UserPersonalManagerDialog view;
    private UserLogin userLogin;
    private boolean edit;

    public UserPersonalManagerController(UserPersonalManagerDialog view, UserLogin userLogin) throws SQLException {
    ...
    private void infoTableFiltro() throws SQLException {
        ...
        if (userLogin.getUsuarioAdmin() != null) {
            ...
            String sql = "SELECT * FROM UsuarioPersonal WHERE dni Like ? OR name Like ?";
    ...
    private void infoTable() throws SQLException {
        ...
        if (userLogin.getUsuarioAdmin() != null) {
            ...
            String sql = "SELECT * FROM UsuarioPersonal";
    ...
    //Método para eliminar selección de la tabla
    ...
    //Método para obtener selección de la tabla
            ...
            public void mouseClicked(MouseEvent e) {
                if (view.getCellInfo().isEmpty() == false) {
                    ...
                    try {
                        ...
                        Logger.getLogger(UserPersonalManagerController.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        ...
        ActionListener al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (view.getUserTable().getSelectedRowCount() > 1) {
                    ...
                    if (value == 0) {
                        try {
                            UsuarioPersonal usuario = obtenerUsuario(selected);
    ...
    //Método para obtener el usuario por el dni
    private UsuarioPersonal obtenerUsuario(String dni) throws SQLException {
        ConnectMdb connMdb = new ConnectMdb();
        Connection conn = connMdb.getConnection();
    ...
    private boolean addUser() throws SQLException {
        ...
        UsuarioPersonal usuario = new UsuarioPersonal(dni, user, password, name, email, address, telefono, ca);

        if (!validarEmail(email) || !validarTelefono(telefono) || !ValidarCampos() || !ValidadorDni(dni)) {

            JOptionPane.showMessageDialog(view, "Comprueba que todo este correcto: \n\t"
                    + "- Todos los campos rellenados.\n\t"
                    + "- El formato del dni es 12345678X\n\t"
                    + "- El formato del teléfono es 123456789\n\t"
                    + "- El formato del email es x@x.x");
            return false;

        } else if (comprobarUsuarioAñadir(usuario)) {
            JOptionPane.showMessageDialog(view, "El usuario el dni ya existe.");
            return false;
        } else {
            String passwordEncriptada = encriptar(password);
            String sql = "Insert into UsuarioPersonal (dni, user, password, email, address, name, telefono, ca) VALUES (?,?,?,?,?,?,?,?)";
    ...
    //Método para validar que los campos esten cubiertos.
    ...
    //Método para comprobar que el usuario no exista
    public static boolean comprobarUsuarioAñadir(UsuarioPersonal user) throws SQLException {
        ConnectMdb connMdb = new ConnectMdb();
        Connection conn = connMdb.getConnection();

        String sqlProfesional = "SELECT * FROM UsuarioProfesional WHERE user = ?";
        String sqlPersonal = "SELECT * FROM UsuarioPersonal WHERE user = ? OR dni = ? ";
        ...
        stmtPersonal.setString(2, user.getDni());
    ...
    public boolean eliminarUsuario(UsuarioPersonal user) throws SQLException {
        ConnectMdb connMdb = new ConnectMdb();
        Connection conn = connMdb.getConnection();

        String sql = "DELETE FROM UsuarioPersonal WHERE dni = ?";
    ...
    //Método para inicializar los componentes de la forma deseada.
    ...
    //Método para comprobar el usuario a editar y que no exista otro.
    ...
    private boolean editUsuario() throws SQLException {
        ...
        String selected = view.getCellInfo();
        UsuarioPersonal original = obtenerUsuario(selected);
        ...
        String sql = "UPDATE UsuarioPersonal SET dni=?, user=?, email=?, address=?, name=?, telefono=?, ca=? WHERE dni = ?";
        PreparedStatement stmt = conn.prepareStatement(sql);

        stmt.setString(1, view.getTextDniTextField());
        ...
        stmt.setString(8, view.getTextDniTextField());
    ...
    //Método para obtener información del usuario editado
