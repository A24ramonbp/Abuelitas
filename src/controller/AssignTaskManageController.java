 ...
 * @author rpbp
 */
public class AssignTaskManageController {

    private final AssignTaskManagerDialog view;
    private UserLogin userLogin;
    private boolean edit;

    public AssignTaskManageController(AssignTaskManagerDialog view, UserLogin userLogin) throws SQLException {
    ...
    public void printProfesional() throws IOException, SQLException {
        ...
        try {
            ...
            JasperReport report = JasperCompileManager.compileReport(
                    ClientManageController.class.getClassLoader().getResourceAsStream("reports/TareasAsignadas.jrxml")
            );
    ...
    public void printCuidador() throws IOException, SQLException {
        ...
        try {
            ...
            Files.createDirectories(Paths.get(currentDirectory + "/Reports"));
            JasperReport report = JasperCompileManager.compileReport(
                    ClientManageController.class.getClassLoader().getResourceAsStream("reports/TareasAsignadas2.jrxml")
            );
    ...
    private void dataMedicos2ComboBox() throws SQLException {
        view.addItemMedicoComboBox("-Unasigned-");
        if (userLogin.getUsuarioProfesional() != null) {
            ConnectMdb connMdb = new ConnectMdb();
            Connection conn = connMdb.getConnection();

            String sql = "SELECT * FROM UsuarioCuidador WHERE rol <> 'Cuidador' and code = ? AND especialidad";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, userLogin.getUsuarioProfesional().getCenterCode());
            ResultSet result = stmt.executeQuery();
